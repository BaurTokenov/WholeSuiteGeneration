...
public class ga {
    public static TestSuite performGA(String targetClassPath, String targetClassName, int population, int generations,
            int maxTestSuiteLen, int maxTestCaseLen) {
        ...
        for (int i = 0; i < generations; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                TestCaseMutator.mutateTestSuite(p1);
                TestCaseMutator.mutateTestSuite(p2);
                testSuites.add(p1);
                testSuites.add(p2);
            }
            ...
            System.out.println(String.format("Generation: %d\nFirst Coverage: %s\nLast Coverage: %s\n\n", i,
                    testSuites.get(0).calculateFitness(), testSuites.get(testSuites.size() - 1).calculateFitness()));
